...
function validateQuestionnaire(params: AskUserQuestionParams): { ok: true } | { ok: false; error: string } {
  ...
  if (params.questions.length > MAX_QUESTIONS) return { ok: false, error: `Error: At most ${MAX_QUESTIONS} questions are allowed per invocation` };

  const seenQuestions = new Set<string>();
  for (const q of params.questions) {
    if (seenQuestions.has(q.question)) return { ok: false, error: "Error: Question text must be unique within an invocation" };
    seenQuestions.add(q.question);
  }

  const reservedLabelSet: ReadonlySet<string> = new Set(RESERVED_LABELS);
  for (const q of params.questions) {
    if (q.options.length < MIN_OPTIONS) return { ok: false, error: `Error: Each question requires at least ${MIN_OPTIONS} options` };
    const seenLabels = new Set<string>();
    for (const o of q.options) {
      if (reservedLabelSet.has(o.label)) return { ok: false, error: `Error: Option label is reserved (${RESERVED_LABELS.join(", ")})` };
      if (seenLabels.has(o.label)) return { ok: false, error: "Error: Option labels must be unique within a question" };
