...
export const appStyles = css`
  ...
  /* --pi-visual-viewport-height (set from window.visualViewport by VisualViewportHeightController) shrinks the shell
     to fit above the on-screen keyboard on browsers/WebViews that don't resize the layout viewport (dvh) for it. */
  :host { --pi-app-safe-area-bottom: 0px; position: fixed; top: 0; right: 0; left: 0; display: block; height: min(100dvh, var(--pi-visual-viewport-height, 100dvh)); box-sizing: border-box; overflow: hidden; padding: env(safe-area-inset-top) env(safe-area-inset-right) var(--pi-app-safe-area-bottom) env(safe-area-inset-left); color: var(--pi-text); background: var(--pi-bg); font: 14px system-ui, sans-serif; }
